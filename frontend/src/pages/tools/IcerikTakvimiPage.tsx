...
export function IcerikTakvimiPage() {
  ...
  // Polling: async job tamamlanınca sonucu al
  useEffect(() => {
  ...
  return (
    <ToolShell
      ...
      title="İçerik Takvimi Üreticisi"
      icon="📅"
      description="Sosyal medya için hazır içerik takvimi oluşturun. Haftalık veya aylık plan, platform bazında içerikler."
      hasResult={hasResult}
      formHasInput={!!form.isletme_adi.trim()}
    >
      {({ isFormOpen }) => (
        <>
          {/* ── Form ───────────────────────────────────────────── */}
          ...
          {hasResult && result && (
            <div className="print-results">
              ...
              <button
                ...
                className="mt-4 text-sm text-gray-400 underline text-center block w-full no-print"
              >
                Yeni takvim oluştur
              </button>
            </div>
